...
export const api = {
  items: {
    ...
    updateStatus: {
      ...
      input: z.object({
        status: z.enum(['reported', 'retrieved', 'donated', 'claimed', 'pending_verification', 'verified', 'resolved']).optional(),
    ...
    // Claim route: submit a claim for an item (found item)
    claim: {
      method: 'POST' as const,
      ...
      responses: {
        201: z.object({ claimId: z.number(), matchScore: z.number(), message: z.string() }),
        400: errorSchemas.validation,
        404: errorSchemas.notFound,
      },
    },
